...
<script runat="server">
	...
	'System.Threading.Thread.CurrentThread.CurrentUICulture=MyCulture
        ...
        If Not IsPostBack Then
            ...
            Dim objCmd As New SqlCommand(strBName, objConnection)
            ...
            Dim objCmdCity As New SqlCommand(strCity, objConnection)
    
            'Create/Populate DataReader
            Dim objDRCity As SqlDataReader
            ...
            Dim objCmdReg As New SqlCommand(strReg, objConnection)
    
            'Create/Populate DataReader
            Dim objDRReg As SqlDataReader
            ...
            Dim objCmdStyle As New SqlCommand(strStyle, objConnection)
    
            'Create/Populate DataReader
            Dim objDRStyle As SqlDataReader
            ...
            strSQLR += "WHERE tblBrand.BName = tblBrewery.Number AND tblBrand.ComDate > getDate()-60 "
            strSQLR += "ORDER BY tblBrand.ComDate DESC"
            Dim objAdapterR As New SqlDataAdapter(strSQLR, objConnection)
            ...
            strSQLE += "ExpiryDate FROM tblNewsEvents WHERE Eventitem=1 AND ExpiryDate>getDate() ORDER BY EventDate"
            Dim objAdapterE As New SqlDataAdapter(strSQLE, objConnection)
            ...
            strSQLN += "ExpiryDate FROM tblNewsEvents WHERE Eventitem=0 AND ExpiryDate>getDate() ORDER BY EntryDate DESC"
            ...
            strSQLB += "FROM tblBreweryComment, tblBrewery WHERE tblBreweryComment.BName = tblBrewery.Number AND tblBreweryComment.DateIn > getDate()-60 "
            strSQLB += "ORDER BY tblBreweryComment.DateIn DESC"
            Dim objAdapterB As New SqlDataAdapter(strSQLB, objConnection)
            ...
            strSQLBB += "FROM tblBrewery, tblBrand, tblBrandComment WHERE tblBrand.BdNumber = tblBrandComment.Brand AND tblBrand.BName = tblBrewery.Number AND tblBrandComment.DateCom > getDate()-60 "
            strSQLBB += "ORDER BY tblBrandComment.DateCom DESC"
            Dim objAdapterBB As New SqlDataAdapter(strSQLBB, objConnection)
            ...
            objConnection.Open()
            Dim strSQLf As String = "Select TOP 1 * FROM tblFeature WHERE fshow = 1 ORDER BY fDate DESC"
            Dim objAdapterF As New SqlDataAdapter(strSQLf, objConnection)
            ...
            Dim objBrewery As New SqlCommand(strBrewery, objConnection)
            Dim objBrewpub As New SqlCommand(strBrewpub, objConnection)
            Dim objClosed As New SqlCommand(strClosed, objConnection)
            Dim objBrands As New SqlCommand(strBrands, objConnection)
            Dim objBrandCom As New SqlCommand(strBrandComment, objConnection)
            Dim objBreweryCom As New SqlCommand(strBreweryComment, objConnection)
